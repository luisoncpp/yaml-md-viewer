...
| Sandboxed preview | `src/preview.ts` | Generated HTML is assigned to `iframe.srcdoc`, with `sandbox="allow-scripts"`. A preview-only CSP blocks network, objects, frames, media, and forms while allowing compiler-owned inline styling/scripts. Never export the injected CSP wrapper. |
